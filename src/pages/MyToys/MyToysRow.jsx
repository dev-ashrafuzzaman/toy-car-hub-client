...
const MyToysRow = ({ toys , handleToyDelete}) => {
    ...
    return (
        <tr className="hover:bg-[#57c3e42f]">
            ...
            <th>
              <Link to={`/toy-details/${_id}`}> <button className="btn btn-ghost bg-green-600 font-semibold text-white hover:text-black btn-xs">Details</button></Link>
              <Link to={`/update-toy-details/${_id}`}> <button className="btn btn-ghost bg-[#57c4e4] font-semibold text-white hover:text-black btn-xs">Update</button></Link>
            </th>
        </tr>
    );
};
